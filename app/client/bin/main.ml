...
let run_client ~host ~port ~participant_name =
  ...
  don't_wait_for
    (Pipe.iter_without_pushback reader ~f:(fun event ->
       ...
       | _ -> print_endline [%string "[MD] %{Protocol.format_event event}"]));
  print_endline
    [%string
      {|
...
market-data feed.|}];
  let rec loop () =
    ...
    | `Ok line ->
      let output = Exchange_command.parse line in
      (match output with
       | Error _ -> loop ()
       | Ok (Book symbol) ->
         let%bind result =
       ...
       | Ok (Subscribe symbol) ->
         let%bind result =
           Rpc.Pipe_rpc.dispatch Rpc_protocol.market_data_rpc conn [ symbol ]
         in
         (match result with
